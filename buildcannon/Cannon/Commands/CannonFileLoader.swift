...
class CannonFileLoader {
    ...
    func assign(file: CannonFile) {
        ...
        if self.checkParameter(value: file.target, commandName: Parameter.target.name) {
            Application.processParameters.append(DoubleDashComplexParameter.init(parameter: Parameter.target.name, composition: file.target!))
        }
